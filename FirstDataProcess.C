...
void FirstDataProcess(string input = "CONFIG/fp_config_file.txt")
{ /* Función principal de esta macro. Las direfentes funciones _Input() llaman al archivo de configuracion e importan las variables pertinentes */
  ...
  for (int run=irun; run<=frun; run++) for (int ch:channels) ReadAndDumpBinary(inputpath+Form("run%02i/wave%i.dat",run,ch),outputpath+Form("run%02i_ch%i.root",run,ch));
