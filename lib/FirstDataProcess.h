...
public:
  ...
  bool LoadBinary(int len, int EN, ULong64_t triggtstmp, vector<int> adc)
  {
    Length = len;
    EventNumber = EN;
  ...
  bool Load(ifstream *ifs, bool DEBUG = false, bool TS = false)
  {
    ...
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // Record Length: 3072
    if (ifs->eof())
      return false;
    ...
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // BoardID:  0
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // Channel: 0
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // Event Number: 57776
    ...
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // Pattern: 0x0000
    getline(*ifs, s);
    if (DEBUG)
    {
      cout << s << endl;
    } // Trigger Time Stamp: 3004795797  2756456643
    s = s.substr(20, s.length() - 20);
    TriggerTimeStamp = stol(s);
    if (DEBUG)
      cout << "TriggerTimeStamp: " << TriggerTimeStamp << endl;
    getline(*ifs, s);
    if (DEBUG)
      cout << s << endl; // DC offset (DAC): 0x0CCC
    if (TS)
    {
      getline(*ifs, s);
      s = s.substr(14, s.length() - 14);
      PCTimeStamp = stol(s);
      if (DEBUG)
        cout << "PC TriggerTimeStamp: " << PCTimeStamp << endl; // PC TriggerTimeStamp: 1594803786
      getline(*ifs, s);
      if (DEBUG)
        cout << s << endl; // ComputerTime Format 2: Wed Jul 15 11:03:06 2020
      getline(*ifs, s);
      if (DEBUG)
        cout << s << endl; // DC offset (DAC): 0x0CCC
      // lets_pause();
    ...
    for (int i = 0; i < Length; i++)
    {
      ...
      if (s == "")
      {
        return false;
        if (DEBUG)
          cout << "File CUT, end run here" << endl;
      }
    ...
    if (DEBUG)
      cout << "ADC samples " << ADCs.size() << " " << Length << endl;
    return true;
    //    for(auto i : ADC) cout << i << " ";
    //    cout << endl;
  }

  bool LoadOscilloscope(TString file, bool DEBUG = false, int Length = 52)
  {
    ifstream ifs;
    ifs.open(file);
    if (DEBUG)
    ...
    if (DEBUG)
      cout << "ADC samples " << ADCd.size() << " " << Length << endl;
    if (DEBUG)
...
void Dump(std::vector<Waveform_t> *w, string file, bool Osci = false)
{
  ...
  TBranch *_bn_u;
  if (Osci)
    _bn_u = t->Branch("ADC", &_ud);
  else
    _bn_u = t->Branch("ADC", &_us);
...
int ReadAndDumpBinary(string file, string outfile)
{
  ...
  while (1)
  {
    ...
    if (wv.size() % 20000 == 0)
    {
      cout << "Loaded waveform " << wv.size() << endl;
      break;
    }
  }
  fclose(pFile);
...
int ReadAndDumpBinaryLight(string file, string outfile)
{
  ...
  while (1)
  {
    ...
    if (counter % 20000 == 0)
    {
      cout << "Loaded waveform " << counter << endl;
    }
    //~ if(wv.size()%100000==0){ cout << "Loaded waveform " << wv.size()<< endl;break;}
  }
  fclose(pFile);
